...
async def export_case_json(
    ...
    sources = (
        (await session.execute(
            select(models.Source).where(models.Source.case_id == case_id)
        )).scalars().all()
    )
    entities = (
        (await session.execute(
            select(models.Entity).where(models.Entity.case_id == case_id)
        )).scalars().all()
    )
    relationships = (
        (await session.execute(
    ...
    data = {
        ...
        "sources": [
            {"id": s.id, "title": s.title, "kind": s.kind, "body": s.body,
             "citation": s.citation, "reliability": s.reliability, "collected_at": s.collected_at}
            for s in sources
        ],
        "entities": [
            {"id": e.id, "type": e.type, "value": e.value, "display": e.display,
             "confidence": e.confidence, "source_ids": e.source_ids,
             "properties": e.properties, "notes": e.notes,
